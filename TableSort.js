...
// hjt1946 2025-01-01 : Bug in der Sortierfunktion bei Tabelle mit Klasse dezimalpunkt --> betrifft immer alle Spalten. Gehört in TH.
// Fehler, Hinweise:
// 1. Embedded STYLE wird nicht erzeugt, wenn <style id="JB_stylesheet_tableSort"> vorhanden ist.
// 2. CSS nicht kompatibel mit Darkmode und Systemtheme.
// 3. In CAPTION wird leeres SPAN ignoriert und immer BR SPAN angefügt. --> BR entfernt, SPAN durch SMALL ersetzt, Zeilenumbruch nutzen.
// 4. clip() muss durch clip-path: path(m0,0h1v1h-1z); ersetzt werden.
...
//				cells_in_thead[col].abbr = (cells_in_thead[col].abbr==sort_info.asc)?sort_info.desc:sort_info.asc;
		...
		console.log("debugmodus: " + "'" + debugmodus + "'");
    // debugmodus = true;
      ...
      return null;
		...
		if(!document.getElementById("JB_stylesheet_tableSort")) {
			...
			const stylestring = '.sortbutton { width: 100%; height: 100%; border: 0px none; background-color: unset; font: inherit; color: inherit; text-align: inherit; padding: 0; cursor: pointer; } '
			 + 'table.sortierbar thead th span.visually-hidden, table[sortable] thead th span.visually-hidden { position: absolute !important; clip-path: path(m0,0h1v1h-1z) !important; padding: 0 !important; border: 0 none !important; height: 1px !important; width: 1px !important; overflow: hidden !important; white-space: nowrap !important; } '
